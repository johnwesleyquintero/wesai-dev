...
const AlertTriangleIcon: React.FC = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-red-400 mr-3 h-6 w-6">
...
const CodeBlock: React.FC<CodeBlockProps> = ({ language, code }) => {
    ...
    return (
        <div className="mb-4 last:mb-0">
            <div className="flex justify-between items-center bg-slate-900 px-4 py-2 rounded-t-md border-b border-slate-700">
            ...
            <pre className="bg-slate-800 p-4 rounded-b-md text-sm text-slate-300 whitespace-pre-wrap overflow-x-auto"><code>{code}</code></pre>
        </div>
    );
};
...
const InitialState: React.FC = () => (
    <div className="text-slate-500 flex flex-col items-center justify-center h-full text-center p-4">
        <CubeIcon className="text-slate-700 mb-4" />
        <h3 className="text-lg font-semibold text-slate-400">Design Canvas</h3>
        <p className="max-w-xs">From prompt to prototype. Describe an interface and watch it materialize.</p>
    </div>
);
...
const OutputDisplay: React.FC<OutputDisplayProps> = ({ response, isLoading, error }) => {
  ...
  const renderContent = () => {
    if (isLoading) {
        return (
            <div className="flex justify-center items-center h-full bg-slate-800/50">
                <LoadingSpinner />
            </div>
        );
    }
    if (error) {
      return (
        <div className="text-red-400 bg-red-900/50 p-4 rounded-md m-4 flex items-start">
            <AlertTriangleIcon/>
            <div>
    ...
    if (response) {
      return (
        <div className="flex flex-col h-full">
          {activeTab === 'preview' ? (
            <div style={checkerboardStyle} className="w-full h-full bg-white rounded-b-md">
                <iframe
                srcDoc={srcDoc}
                title="Preview"
            ...
            <div className="flex-grow overflow-auto p-4 bg-slate-800/50 rounded-b-md">
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-lg flex flex-col h-full">
        <div className="flex items-center justify-between border-b border-slate-700 bg-slate-900/50 p-2 rounded-t-lg">
            <h2 className="text-lg font-semibold text-slate-200 px-2">Output</h2>
            {response && (
